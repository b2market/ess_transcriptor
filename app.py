...
st.set_page_config(page_title="Russian Text Enhancer",
                   page_icon="📝",
                   layout="wide")
...
if not api_key:
    ...
    st.warning(
        "OpenAI API key not found in environment variables. Please add OPENAI_KEY to your Secrets tab."
    )
    st.info("Available environment variables: " + ", ".join([
        key for key in os.environ.keys()
        if "OPENAI" in key or "API" in key or "KEY" in key
    ]))
...
if text_input:
    ...
    chunking_method = st.radio(
        "Chunking method:", ["By Sentence Count", "By Character Count"],
    ...
    if chunking_method == "By Character Count":
        chunk_size = st.slider(
            ...
            step=500,
            help=
            "Size of text chunks to process separately. Larger chunks provide better context but may hit API limits."
        )
        ...
        line_count = st.slider(
            ...
            step=5,
            help=
            "Number of sentences to include in each chunk. 50 sentences is recommended for optimal processing."
        )

    model = st.selectbox("Select OpenAI model", ["gpt-4o"],
...
st.markdown("---")
st.markdown(
    "**Note:** This application uses OpenAI's GPT-4o to enhance Russian transcribed text."
)
