...
def process_text_with_chatgpt(text, api_key=None, model="gpt-4o"):
    ...
    1. Каждую отдельную мысль размещай на отдельной строке. Если мысль сложная, можно сделать перенос строки для улучшения восприятия.
    2. МИНИМАЛЬНО используй подзаголовки - только для крупных смысловых блоков.
    3. Избегай абзацев из нескольких предложений - лучше каждая новая мысль с новой строки.
    4. Перечисления и примеры оформляй в лаконичные списки.
    5. Все роли, термины и ключевые идеи должны остаться без изменений.
    
    Важные правила:
    - Сохраняй авторский стиль и интонацию — не перефразируй.
    - Оставляй все примеры и пояснения в исходном виде.
    - Не упрощай и не обобщай текст — все детали и нюансы важны.
    - Не создавай лишних заголовков или подзаголовков.
    
    Цель: текст должен читаться легко, с четким разделением мыслей по строкам, а не абзацам.
    Ответь в простом текстовом формате с минимальным использованием markdown. Используй только выделение для важных моментов и списки для перечислений.
